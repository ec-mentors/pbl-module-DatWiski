...
export const useAuth = () => {
  ...
  const hasValidToken = Boolean(token && !tokenStorage.isTokenExpired(token));

  const { data: authStatus, isLoading, error } = useQuery<AuthStatus>({
    queryKey: ['auth-status'],
    queryFn: async () => {
      const currentToken = tokenStorage.getToken();
      if (!currentToken || tokenStorage.isTokenExpired(currentToken)) {
  ...
  const isAuthenticated = hasValidToken && ((authStatus as any)?.authenticated || false);
  ...
  useEffect(() => {
    const refreshTokenIfNeeded = async () => {
      if (token && tokenStorage.isTokenExpiringSoon(token, 10)) {
        try {
          const refreshResult = await tokenStorage.refreshToken();
          if (refreshResult) {
            login(refreshResult.token, refreshResult.user);
          } else {
  ...
  return {
    isAuthenticated,
    username: localUser?.name || (authStatus as any)?.username,
    user: localUser,
    token,
    login,
    logout,
